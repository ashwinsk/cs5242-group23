...
from utilities import custom_loss_sig, accuracy_ones

np.random.seed(5242)

wandb.init(project="cs5242", entity="cs5242-group-23", name="pafa ones", notes="Using 90% of dataset for train",
           config={"epochs": 8, "batch_size": 80, 'lr': 0.002, 'val_batch_size': 50})
...
if __name__ == '__main__':
    ...
    model.compile(loss=custom_loss_sig(2.), optimizer=adam, metrics=['accuracy', accuracy_ones])
    ...
    filepath = os.path.join(wandb.run.dir, "saved-model-{epoch:02d}-{val_accuracy_ones:.2f}.hdf5")
    checkpoint = ModelCheckpoint(filepath, monitor='val_accuracy_ones', verbose=0, save_best_only=False,
